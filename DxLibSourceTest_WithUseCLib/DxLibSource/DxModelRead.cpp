...
static MV1_FRAME_R *MV1RAddFrameBase( MV1_MODEL_R *ReadModel, const char *NameA, const wchar_t *NameW, MV1_FRAME_R *Parent )
{
	...
	if( NameA == NULL )
	{
		ConvString( ( const char * )NameW, WCHAR_T_CHARCODEFORMAT, ( char * )TempBuffer, sizeof( TempBuffer ), CHAR_CHARCODEFORMAT ) ;
		NameA = ( const char * )TempBuffer ;
	}
	else
	if( NameA != NULL && CHAR_CHARCODEFORMAT != ReadModel->CharCodeFormat )
	{
		ConvString( ( const char * )NameA, ReadModel->CharCodeFormat, ( char * )NameATempBuffer, sizeof( NameATempBuffer ), CHAR_CHARCODEFORMAT ) ;
		NameA = ( const char * )NameATempBuffer ;
	}
#endif
	if( NameW == NULL )
	{
		ConvString( ( const char * )NameA, CHAR_CHARCODEFORMAT, ( char * )TempBuffer, sizeof( TempBuffer ), WCHAR_T_CHARCODEFORMAT ) ;
	...
	bool OpenFailedReturn )
{
	...
	if( NameA == NULL )
	{
		ConvString( ( const char * )NameW, WCHAR_T_CHARCODEFORMAT, ( char * )TempNameBuffer, sizeof( TempNameBuffer ), CHAR_CHARCODEFORMAT ) ;
		NameA = ( const char * )TempNameBuffer ;
	}
	else
	if( NameA != NULL && CHAR_CHARCODEFORMAT != ReadModel->CharCodeFormat )
	{
		ConvString( ( const char * )NameA, ReadModel->CharCodeFormat, ( char * )TempNameABuffer, sizeof( TempNameABuffer ), CHAR_CHARCODEFORMAT ) ;
		NameA = ( const char * )TempNameABuffer ;
	}
#endif
	if( NameW == NULL )
	{
		ConvString( ( const char * )NameA, CHAR_CHARCODEFORMAT, ( char * )TempNameBuffer, sizeof( TempNameBuffer ), WCHAR_T_CHARCODEFORMAT ) ;
	...
	if( ColorFilePathA != NULL || ColorFilePathW != NULL )
	{
		...
		{
			ConvString( ( const char * )ColorFilePathW, WCHAR_T_CHARCODEFORMAT, ( char * )TempColorFilePathBuffer, sizeof( TempColorFilePathBuffer ), CHAR_CHARCODEFORMAT ) ;
			ColorFilePathA = ( const char * )TempColorFilePathBuffer ;
		}
		else
		if( ColorFilePathA != NULL && CHAR_CHARCODEFORMAT != ReadModel->CharCodeFormat )
		{
			ConvString( ( const char * )ColorFilePathA, ReadModel->CharCodeFormat, ( char * )TempColorFilePathABuffer, sizeof( TempColorFilePathABuffer ), CHAR_CHARCODEFORMAT ) ;
			ColorFilePathA = ( const char * )TempColorFilePathABuffer ;
		}
#endif
		if( ColorFilePathW == NULL )
		{
			ConvString( ( const char * )ColorFilePathA, CHAR_CHARCODEFORMAT, ( char * )TempColorFilePathBuffer, sizeof( TempColorFilePathBuffer ), WCHAR_T_CHARCODEFORMAT ) ;
	...
	if( AlphaFilePathA != NULL || AlphaFilePathW != NULL )
	{
		...
		{
			ConvString( ( const char * )AlphaFilePathW, WCHAR_T_CHARCODEFORMAT, ( char * )TempAlphaFilePathBuffer, sizeof( TempAlphaFilePathBuffer ), CHAR_CHARCODEFORMAT ) ;
			AlphaFilePathA = ( const char * )TempAlphaFilePathBuffer ;
		}
		else
		if( AlphaFilePathA != NULL && CHAR_CHARCODEFORMAT != ReadModel->CharCodeFormat )
		{
			ConvString( ( const char * )AlphaFilePathA, ReadModel->CharCodeFormat, ( char * )TempAlphaFilePathABuffer, sizeof( TempAlphaFilePathABuffer ), CHAR_CHARCODEFORMAT ) ;
			ColorFilePathA = ( const char * )TempAlphaFilePathABuffer ;
		}
#endif
		if( AlphaFilePathW == NULL )
		{
			ConvString( ( const char * )AlphaFilePathA, CHAR_CHARCODEFORMAT, ( char * )TempAlphaFilePathBuffer, sizeof( TempAlphaFilePathBuffer ), WCHAR_T_CHARCODEFORMAT ) ;
							...
							ColorImage.ColorData.BlueWidth  > 0 ) ) )
					{
						...
						for( i = 0 ; i < ColorImage.Height ; i ++ )
						{
							for( j = 0 ; j < ColorImage.Width ; j ++ )
							{
								...
								NS_GetPixelBaseImage( &ColorImage, j, i, &r, &g, &b, NULL ) ;
