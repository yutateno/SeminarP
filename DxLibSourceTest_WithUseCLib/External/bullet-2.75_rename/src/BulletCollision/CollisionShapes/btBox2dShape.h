...
///The D_btBox2dShape D_is a box primitive around the origin, its sides axis aligned with length specified by half extents, in local shape coordinates. When used as part of a D_btCollisionObject or D_btRigidBody it D_will be an oriented box in world space.
class D_btBox2dShape: public D_btPolyhedralConvexShape
{

	//D_btVector3	m_boxHalfExtents1; //use m_implicitShapeDimensions instead

	D_btVector3 m_centroid;
	D_btVector3 m_vertices[4];
	D_btVector3 m_normals[4];
...
public:
	...
	D_btBox2dShape( const D_btVector3& boxHalfExtents) 
		...
		m_centroid(0,0,0)
	{
		...
		m_shapeType = D_BOX_2D_SHAPE_PROXYTYPE;
		D_btVector3 margin(getMargin(),getMargin(),getMargin());
	...
	const D_btVector3* getVertices() const
	{
		return &m_vertices[0];
	}

	const D_btVector3* getNormals() const
	{
		return &m_normals[0];
	}
	...
	const D_btVector3& getCentroid() const
	{
		return m_centroid;
	}
