...
void D_gim_contact_array::merge_contacts(
	const D_gim_contact_array & contacts, bool normal_contact_average)
{
	clear();
	...
	D_gim_array<D_GIM_RSORT_TOKEN> keycontacts(contacts.size());
	...
	//sort keys
	D_gim_heap_sort(keycontacts.D_pointer(),keycontacts.size(),D_GIM_RSORT_TOKEN_COMPARATOR());

	// Merge contacts

	D_GUINT coincident_count=0;
	D_btVector3 coincident_normals[D_MAX_COINCIDENT];

	D_GUINT last_key = keycontacts[0].m_key;
	D_GUINT key = 0;

	push_back(contacts[keycontacts[0].m_value]);
	D_GIM_CONTACT * pcontact = &back();
...
void D_gim_contact_array::merge_contacts_unique(const D_gim_contact_array & contacts)
{
	clear();
	...
	D_GIM_CONTACT average_contact = contacts.back();

	for (D_GUINT i=1;i<contacts.size() ;i++ )
	{
	...
	//divide
	D_GREAL divide_average = 1.0f/((D_GREAL)contacts.size());
