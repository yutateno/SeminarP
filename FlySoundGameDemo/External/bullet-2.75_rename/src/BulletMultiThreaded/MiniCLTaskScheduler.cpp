...
D_MiniCLTaskScheduler::D_MiniCLTaskScheduler(D_btThreadSupportInterface*	threadInterface,  int maxNumOutstandingTasks)
...
D_MiniCLTaskScheduler::~D_MiniCLTaskScheduler()
{
...
void	D_MiniCLTaskScheduler::initialize()
{
#ifdef DEBUG_SPU_TASK_SCHEDULING
	printf("D_MiniCLTaskScheduler::initialize()\n");
...
void D_MiniCLTaskScheduler::issueTask(int firstWorkUnit, int lastWorkUnit,int kernelProgramId,char* argData,int* argSizes)
{

#ifdef DEBUG_SPU_TASK_SCHEDULING
	printf("D_MiniCLTaskScheduler::issueTask (m_currentTask= %d\)n", m_currentTask);
#endif //DEBUG_SPU_TASK_SCHEDULING

	m_taskBusy[m_currentTask] = true;
	m_numBusyTasks++;

	D_MiniCLTaskDesc& taskDesc = m_spuSampleTaskDesc[m_currentTask];
	{
		...
		for (int i=0;i<D_MINI_CL_MAX_ARG;i++)
		{
			...
			{
				memcpy(&taskDesc.m_argData[i],&argData[D_MINICL_MAX_ARGLENGTH*i],taskDesc.m_argSizes[i]);
			}
		}
	}
...
void D_MiniCLTaskScheduler::postProcess(int taskId, int outputSize)
{

}


void D_MiniCLTaskScheduler::flush()
{
#ifdef DEBUG_SPU_TASK_SCHEDULING
