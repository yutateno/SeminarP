...
HRESULT CD3DXSKINMESH::Init(CD3DXSKINMESH_INIT* si)
{
	...
	//ブロブからバーテックスシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"Skin.", NULL, NULL, "VSSkin", "vs_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
...
HRESULT CD3DXSKINMESH::CreateFromX(CHAR* szFileName)
{
	...
	for (int i = 0; i<m_dwNumVert; i++)
	{
		D3DXVECTOR2 v12 = m_pD3dxMesh->GetUV(i);
		D3DXVECTOR3 v = D3DXVECTOR3(v12.x, v12.y, 0);
		pvVB[i].vTex.x = v.x;
		pvVB[i].vTex.y = v.y;
	}
	//マテリアル読み込み
